...
async def resolve_account_number(account_number: str, bank_code: str):
    response = await httpx.AsyncClient().get(f'https://api.paystack.co/bank/resolve?account_number={account_number}&bank_code={bank_code}',
...
async def get_bank_list(country: str, items_per_page: int = 100, provider: PaymentServiceProviderType = PaymentServiceProviderType.PAYSTACK):
    if items_per_page > 100:
        raise ValueError("items_per_page must be less than 100")
    response = await httpx.AsyncClient().get(f'https://api.paystack.co/bank?country={country}&perPage={items_per_page}',
        headers={'Authorization': f'Bearer {settings.PAYSTACK_SECRET_KEY}'})
    banks = []
    if response.status_code == 200 and response.json().get("status"):
